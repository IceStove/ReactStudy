...
// 프로미스를 기다렸따가 결과를 디스패치 하는 사가
export const createPromiseSaga = (type, promiseCreator) => {
  const [SUCCESS, ERROR] = [`${type}_SUCCESS`, `${type}_ERROR`];
  return function* saga(action) {
    try {
      // 재사용성을 위하여 promiseCreator의 파라미터엔 action.payload 값을 넣도록 설정합니다.
...
// Promise에 기반한 Thunk를 만들어주는 함수입니다.  --> Saga로 변경하면서 사용하지 않음
...
//         dispatch({type, param});
//         try {
//             // 결과물의 이름을 payload라는 이름으로 통일시킵니다.
...
export const reducerUtils = {
    ...
    // 따로 값을 지정하면 null로 바꾸지 않고 다른 값을 유지시킬 수 있습니다.
...
// 특정 id 를 처리하는 Thunk 생성함수 --> Saga로 바꾸면서 사용하지 않는 코드
